...
- 卡号范围选择：输入起始 ICCID 和结束 ICCID，自动生成连续卡号并加入待出库列表
- Excel批量出库
- 卡片列表展示材质列
...
- 输入内容自动去重并实时统计；ICCID 支持 10-30 位字母或数字，其他内容必须在提交前提示，单次最多 10000 条。
...
- 卡号范围选择按 ICCID 尾部数字递增生成，起止卡号前缀必须一致；单次最多 10000 张，最终仍通过库存批量查询接口校验当前可出库状态
...
- 批量查询仅展示当前库存卡；已出库、已激活、已停机等非库存卡号必须进入“未找到”列表展示
- 批量查询未找到和重复输入的卡号默认折叠，通过按钮弹窗查看，一行一个卡号并支持导出
- 库存批量查询模式下，导出库存应导出当前已筛选出的库存卡片，而不是回退到普通筛选条件
...
- 批量查询（最多10000个ICCID）
- 卡片列表已勾选卡片时，批量查询/划拨/备注/续费/补量/停机/复机直接使用勾选卡号，不再要求二次输入 ICCID
...
**卡片列表导出要求**：
- 按当前筛选条件或当前勾选卡片导出，不受列表分页限制
- 导出覆盖卡片列表及卡片详情中已有的运营字段，包括关联账户、卡片类型、流量、本月用量、生命周期、停卡、流量池、出入库、套餐价格、同步和设备信息
- 关联账户至少包含账户 ID、账户名称和登录账号；禁止导出密码、密钥等敏感信息
...
- 配置平台默认出库、续费积分比例。
- 管理独立渠道账号、状态、报备开关、比例覆盖和专属 H5 地址。
- 查看具体渠道、客户、手机号、订单号、ICCID、计佣基数、比例和积分。
- 勾选待结算记录并确认结算。
- 超级管理员的“用户管理－用户列表”增加“推荐渠道”列和渠道筛选，无渠道归属显示“—”；一级平台用户自己的下级用户列表保持原样。

**公开报备 H5**：`/channel/register/:slug`

- 填写客户姓名、手机号、用户情况（设备、场景、规模）和隐私告知确认；用户情况必填 5～500 字。
- 不发送短信验证码。
- 成功后只提示客户档案已创建，不展示内部用户 ID、账号或密码。
- 校验或接口失败必须显示可见反馈，并提供渠道伙伴登录入口。

**渠道积分页面**：`/channel/login`、`/channel/points`

- 使用独立渠道账号登录，不复用平台登录态。
- 展示渠道客户数、累计、已消耗（已结算）、剩余（待结算）和本月积分。
- “推荐客户”页按客户展示用户情况、报备时间、积分笔数及累计、已消耗、剩余积分。
- 逐笔展示客户、订单、卡号、计佣基数、比例、积分和结算状态。
- 渠道只能查询自己的积分数据。

**详细PRD**：`docs/CHANNEL_POINTS_COMMISSION_PRD.md`
