...
- 问题：库存管理列表点击第 2、3 页后，页面仍然显示第一页数据
- 原因：分页组件的翻页事件复用了查询方法，而查询方法会先把 `page` 强制重置为 `1`
- 修复：拆分“查询”“翻页”“修改每页条数”三个事件处理，只有主动查询和修改每页条数时才重置到第一页
- 影响文件：`frontend/src/views/stock/inventory/index.vue`
