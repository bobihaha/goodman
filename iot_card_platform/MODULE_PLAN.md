...
- 到期提醒：每月 10 号、15 号定时发送本月到期卡汇总邮件，按关联用户聚合；正文显示到期数量和时间范围，附件为 Excel 明细，包含运营商、IMSI、ICCID、电话号码、套餐总量、激活日期、到期日期、关联用户、备注、续费价格。
- 用量提醒：流量池阈值提醒和非流量池单卡用量提醒使用同一类表格邮件正文；流量池阈值读取用户在流量池上配置的阈值，非流量池单卡固定按 80%、90%、100% 触发提醒。
