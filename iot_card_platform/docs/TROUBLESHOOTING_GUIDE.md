...
- 回收卡片时是否同时清空 `pool_id`、置 `is_pool_member=0` 并刷新原池统计
- 池成员统计口径是否只包含 `pool_id` 匹配、`is_pool_member=1`、`user_id` 不为空、未删除的卡
- 仪表盘流量池数量、流量池列表、流量池统计是否复用同一套空自动池过滤口径
- 池统计更新逻辑是否执行
- 阈值配置是否存在
- 停卡联动是否被供应商调用异常中断
...
- 规格不匹配导致未真正入池
- 统计刷新方法未被调用
- 回收只清了用户或套餐，未清流量池关系，导致历史自动池残留为 0 卡重复池
- 自动池无有效成员时仍按历史 `card_count` 展示
- 池附加流量月份逻辑造成口径偏差
- 超阈值后供应商 API 调用失败
