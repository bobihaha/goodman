...
- SIMBOSS 接受开关网络请求后，`deviceStatus` 可能延迟更新。平台将“接口已接受但状态尚未收敛”记录为待对账，并继续自动查询供应商状态，不再误记为供应商未收到请求。
- 流量池超限自动停机必须走统一供应商操作入口，写入 `supplier_suspend_operations`；只有供应商状态确认收敛后才同步本地状态。
- 流量池卡列表的网络状态以平台已同步的卡状态生成：已激活为开启、已停机为关闭，其他生命周期状态显示未知；字段缺失时不得默认显示关闭。
