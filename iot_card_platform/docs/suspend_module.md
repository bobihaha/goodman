...
- `card_count` 表示仍在池内的全部有效成员数量，用于展示激活、沉默、停机等状态分布。
- `package_flow` 和 `data_used` 按卡片是否仍在计费累计：已激活卡，以及人工停卡、流量池超限、单卡超量、机卡分离等非到期停机卡，继续贡献池额度与用量。
- 月包流量池的 `data_used` 只汇总 `data_sync_at` 已进入本自然月的卡片；上月遗留或尚未同步的用量按 0 计入本月用量，不能用于触发告警或自动停机。年包仍按套餐有效期累计用量统计。
- 到期停卡、沉默期、测试期、已到期和已注销卡不贡献池额度与用量；到期停卡成功后应及时扣减池额度。
- 卡片加入、移除、回收、停机、复机或供应商状态同步后，必须刷新对应流量池统计；人工停卡刷新后额度不变，到期停卡刷新后额度减少。

#### 供应商停复机对账与展示

- SIMBOSS 流量池卡关停、恢复及流量池超限自动停机统一记录供应商操作流水；禁止绕过操作流水直接调用供应商客户端。
- H5 单卡停机、复机请求提交后的前 3 次对账按 5 秒间隔快速确认，仍未收敛则回退默认 90 秒间隔；供应商响应已明确返回目标状态时，平台当次请求直接收敛本地状态。后台批量及调度操作继续使用默认 90 秒对账间隔，避免集中查询供应商。
